#!/usr/bin/env python3
"""screen_stream3.py — Slot 3 exclusif → /screenhost3 → ws port 8767"""
...
ESP32_URL   = "http://prime-studio.local/screenhost3"
WS_PORT     = 8767           # Port EXCLUSIF slot 3
SLOT_ID     = 3
DEFAULT_FPS = 15
DEFAULT_QUAL = 90
DEFAULT_MON  = 1
...
# ── macOS : vérifie permission enregistrement écran ─────────────
def check_screen_permission():
    if platform.system() != "Darwin":
        return True
    try:
        # Test rapide : tente une capture minuscule
        import mss as _mss
        with _mss.mss() as s:
            s.grab(s.monitors[0])
        return True
...
async def handler(ws):
    ...
    print(f"  ✅  [SLOT 3] {addr[0]}:{addr[1]} ({len(CLIENTS)} actif(s))", flush=True)
    try: await ws.wait_closed()
    finally:
        CLIENTS.discard(ws)
        print(f"  ⚠️  [SLOT 3] Déconnecté {addr[0]}", flush=True)

async def stream_loop(fps, quality, monitor_idx):
    interval = 1.0 / fps
    with mss.mss() as sct:
        mon = sct.monitors[min(monitor_idx, len(sct.monitors)-1)]
        print(f"  📺  [SLOT 3] {mon['width']}x{mon['height']}", flush=True)
...
async def main(fps, quality, monitor_idx):
    ...
    print(f"\n╔══════════════════════════════════════════╗")
    print(f"║  SCREEN STREAM — SLOT 3 (port 8767)      ║")
    print(f"╠══════════════════════════════════════════╣")
    print(f"║  Appareil : {hostname:<28} ║")
    print(f"║  IP       : {ip:<28} ║")
    print(f"║  WS Port  : 8767                         ║")
    print(f"║  Endpoint : /screenhost3                 ║")
    print(f"╚══════════════════════════════════════════╝\n")

    # Vérifie permission écran macOS

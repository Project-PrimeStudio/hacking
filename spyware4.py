#!/usr/bin/env python3
"""screen_stream4.py — Slot 4 exclusif → /screenhost4 → ws port 8768"""
...
ESP32_URL   = "http://prime-studio.local/screenhost4"
WS_PORT     = 8768           # Port EXCLUSIF slot 4
SLOT_ID     = 4
DEFAULT_FPS = 15
DEFAULT_QUAL = 90
DEFAULT_MON  = 1
...
async def handler(ws):
    ...
    print(f"  ✅  [SLOT 4] {addr[0]}:{addr[1]} ({len(CLIENTS)} actif(s))", flush=True)
    try: await ws.wait_closed()
    finally:
        CLIENTS.discard(ws)
        print(f"  ⚠️  [SLOT 4] Déconnecté {addr[0]}", flush=True)

async def stream_loop(fps, quality, monitor_idx):
    interval = 1.0 / fps
    with mss.mss() as sct:
        mon = sct.monitors[min(monitor_idx, len(sct.monitors)-1)]
        print(f"  📺  [SLOT 4] {mon['width']}x{mon['height']}", flush=True)
...
async def main(fps, quality, monitor_idx):
    ...
    print(f"\n╔══════════════════════════════════════════╗")
    print(f"║  SCREEN STREAM — SLOT 4 (port 8768)      ║")
    print(f"╠══════════════════════════════════════════╣")
    print(f"║  Appareil : {hostname:<28} ║")
    print(f"║  IP       : {ip:<28} ║")
    print(f"║  WS Port  : 8768                         ║")
    print(f"║  Endpoint : /screenhost4                 ║")
    print(f"╚══════════════════════════════════════════╝\n")

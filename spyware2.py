#!/usr/bin/env python3
"""screen_stream2.py — Slot 2 exclusif → /screenhost2 → ws port 8766"""
...
ESP32_URL   = "http://prime-studio.local/screenhost2"
WS_PORT     = 8766           # Port EXCLUSIF slot 2
SLOT_ID     = 2
DEFAULT_FPS = 15
DEFAULT_QUAL = 90
DEFAULT_MON  = 1
...
async def handler(ws):
    ...
    print(f"  ✅  [SLOT 2] {addr[0]}:{addr[1]} ({len(CLIENTS)} actif(s))", flush=True)
    try: await ws.wait_closed()
    finally:
        CLIENTS.discard(ws)
        print(f"  ⚠️  [SLOT 2] Déconnecté {addr[0]}", flush=True)

async def stream_loop(fps, quality, monitor_idx):
    interval = 1.0 / fps
    with mss.mss() as sct:
        mon = sct.monitors[min(monitor_idx, len(sct.monitors)-1)]
        print(f"  📺  [SLOT 2] {mon['width']}x{mon['height']}", flush=True)
...
async def main(fps, quality, monitor_idx):
    ...
    print(f"\n╔══════════════════════════════════════════╗")
    print(f"║  SCREEN STREAM — SLOT 2 (port 8766)      ║")
    print(f"╠══════════════════════════════════════════╣")
    print(f"║  Appareil : {hostname:<28} ║")
    print(f"║  IP       : {ip:<28} ║")
    print(f"║  WS Port  : 8766                         ║")
    print(f"║  Endpoint : /screenhost2                 ║")
    print(f"╚══════════════════════════════════════════╝\n")
